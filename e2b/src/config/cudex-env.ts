...
import { z } from 'zod'
import { explicitBoolean, httpUrl, invalidEnvironment, nonemptyString, scopedRuntimeEnv, type RuntimeEnvironment } from './shared.js'

const keys = ['CUDEX_API_URL', 'CUDEX_API_KEY', 'CUDEX_VALIDATE_API_KEY', 'CUDEX_DOMAIN', 'CUDEX_PROVIDER_CA_CERTIFICATE',
...
export function loadCudexEnv(runtimeEnv: RuntimeEnvironment = process.env) {
  return createEnv({ server: {
    ...
    CUDEX_VALIDATE_API_KEY: explicitBoolean.default(true),
