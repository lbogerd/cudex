...
interface ServerOptions { host: string; port: number; bearerToken: string; tlsCertPath?: string; tlsKeyPath?: string }
...
async function body(request: IncomingMessage): Promise<unknown> {
  ...
  for await (const chunk of request) { size += chunk.length; if (size > 1024 * 1024) throw new ServiceError(413, 'request too large'); chunks.push(chunk) }
...
export async function startServer(service: ControlPlane, gateway: ExecGateway, options: ServerOptions) {
  const handler = async (request: IncomingMessage, response: ServerResponse) => {
    try {
      ...
      response.end(JSON.stringify({ error: error instanceof Error ? error.message : 'service unavailable' }))
    }
  }
