...
3. Call `spawn_agent` exactly once with `agent_type: "child"`. Tell that child to verify the owner state file, verify the owner `/tmp` marker is absent, write `child-result.txt` as `child-saw-owner-spawn-state`, change `src/message.txt` to `hosted-child-complete`, check its work, and finish normally.
